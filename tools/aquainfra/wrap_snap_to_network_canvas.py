...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', default="", help='Filename where to read input coordinates in GeoJSON')
    ...
    if len(args.input) > 0:
        ...
        with open(INPUTFILE, 'r') as inp:
            multipoint_to_pass_on = geojson.load(inp)
        LOGGER.info('Inputs read from file: %s' % INPUTFILE)
        LOGGER.info('THIS IS THE INPUT: %s' % multipoint_to_pass_on)

    elif len(args.coordinate_geojson) == 0:
